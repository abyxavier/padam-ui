    ...
    <div className="flex flex-col items-center w-full max-w-7xl mx-auto px-6 py-16">
      ...
      <div className="text-center mb-14 max-w-2xl">
        <h1 className=" text-5xl font-bold tracking-tight leading-[1.15] mb-4">
          Find your next
          <span className=" ml-2 bg-gradient-to-r from-violet-400 via-pink-400 to-orange-400 bg-clip-text text-transparent">
            favourite film
          </span>
        </h1>
      ...
      <div className="flex flex-wrap gap-2 mb-12 justify-center">
        {GENRES.map((genre) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all duration-200 ${selectedGenre === genre
      ...
      {/* ── Error State ───────────────────────────────────────── */}
      {isError && (
        <div className="flex flex-col items-center gap-5 mt-12 mb-8 text-center p-8 border border-red-500/20 rounded-2xl bg-red-500/5 max-w-md w-full mx-auto shadow-lg shadow-red-500/5">
          <div className="w-20 h-20 rounded-full bg-red-500/10 border border-red-500/30 flex items-center justify-center shadow-inner shadow-red-500/20">
            <AlertTriangle className="w-10 h-10 text-red-500 opacity-80" />
          </div>
          <div className="space-y-3">
            <h3 className="text-red-400 text-xl font-bold tracking-tight">Something went wrong</h3>
            <p className="text-red-200/70 text-base max-w-sm mx-auto leading-relaxed cursor-default">
              We couldn't fetch recommendations. This is likely due to an issue with the backend server. Please try again later.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="w-full">
            <div className="flex items-center justify-between mb-6">
              <p className="text-sm text-muted-foreground">
                Showing <span className="text-foreground font-medium">{data?.movie_list.length} results</span> for "{query}"
              </p>
